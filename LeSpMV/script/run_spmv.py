...
def Run_CSR_intKernel(excel_path):
    ...
    for matID, name, mtx_path in mtx_info:
        ...
        # 不再需要 --sche是因为 benchmark_spmv_csr 中自动跑四次不同的调度模式
        print(f"Executing: {command}")
...
def Run_BSR_intKernel(excel_path):
    ...
    for matID, name, mtx_path in mtx_info:
        # 构建并运行命令
        command = f"./benchmark_spmv_bsr {mtx_path} --matID={matID} --Index={index} --precision={precision}"
        # 不再需要 --sche是因为 benchmark_spmv_dia 中自动跑四次不同的调度模式
        print(f"Executing: {command}")
...
def Run_SELL_c_sigma_intKernel(excel_path):
    # 读dataset list 并生成数据组：
    mtx_info = Read_TestDataset(excel_path)
...
def Run_SELL_c_R_intKernel(excel_path):
    # 读dataset list 并生成数据组：
    mtx_info = Read_TestDataset(excel_path)
...
def Run_MKL_intKernel(excel_path):
    # 读dataset list 并生成数据组：
    mtx_info = Read_TestDataset(excel_path)
    ...
    for matID, name, mtx_path in mtx_info:
        # 构建并运行命令
        command = f"./baseline_mkl_csr {mtx_path} --matID={matID} --Index={index} --precision={precision}"
        print(f"Executing: {command}")
...
if __name__ == "__main__":
    # Run_COO_intKernel("./SuiteSparse_Matrix.xlsx")
    ...
    # Run_ELL_intKernel("./SuiteSparse_Matrix.xlsx")
    # Run_SELL_intKernel("./SuiteSparse_Matrix.xlsx")
    # Run_SELL_c_sigma_intKernel("./SuiteSparse_Matrix.xlsx")
    # Run_SELL_c_R_intKernel("./SuiteSparse_Matrix.xlsx")
    Run_MKL_intKernel("./SuiteSparse_Matrix.xlsx")
