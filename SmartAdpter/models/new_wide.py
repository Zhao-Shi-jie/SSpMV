...
def train_new_wide_model(model_path,feat_array,label_array):
  combined_model = NewWide()
  Optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
  combined_model.compile(optimizer=Optimizer,
                         loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
                         metrics=['accuracy'])
  combined_model.fit(feat_array, label_array, batch_size=512, epochs=256)  
...
def train_and_get_res_ten_fold():
  ...
  res_dir = "/data1/xionghantao/data/JPDC/prediction_result/new_wide"
  fold_num = 10
  for i in range(fold_num):
    ...
    model_path = "/data1/xionghantao/data/JPDC/ml_models/new_wide_" + str(i)
    
    train_new_wide_model(model_path, feat_array, label_array)
    ...
    evaluate_new_wide_model(model_path, feat_array, label_array, test_data, eva_path, res_path)
