...
from utils.load_MMdata import get_train_density
from utils.load_MMdata import get_test_Density
...
def train_MM_model(model_path, 
                   image_array, feat_array, label_array, 
                   val_image_array, val_feat_array, val_label_array,
                   label_nums):
  ...
  logger_name = "DensityRep_" + ttime +'.csv'
  csv_logger = CSVLogger(logger_name, append=True, separator=' ')
  
  model.fit([feat_array, image_array], 
            label_array, 
            batch_size=64, 
            epochs=512,
            validation_data=([val_feat_array, val_image_array], val_label_array),
            callbacks=[csv_logger])
  
  # tf.keras.models.save_model(model, model_path)
  model.save(model_path)  # 推荐使用这种方式
  print ("Finish Density-Reps Model Saving")

def train_and_get_res(training_data_list, val_data_list, label_suffix, label_nums):

  image_array, feat_array, label_array = get_train_density(training_data_list, label_suffix)
  
  val_image_array, val_feat_array, val_label_array = get_train_density(val_data_list, label_suffix)
  ...
  # 保存模型的目录
  model_path = "/data/lsl/SSpMV/models/SpMV_DensityRep.keras"
...
def evaluate_MM_Adapter(model_path, image_array_test, feat_array_test, label_array_test, test_data, eva_path, res_path):
  
  loaded_model = tf.keras.models.load_model(model_path)
  
  test_loss, test_acc = loaded_model.evaluate([feat_array_test, image_array_test], label_array_test, verbose=2)
  
  f_eva = open(eva_path, "w")
  f_eva.write("Density Evaluating Acc:" + str(test_acc) + "\n")
  f_eva.close()
  ...
  for file_ in test_data:
    ...
    predictions = loaded_model([feat, img])
...
def test_model(test_data_list, label_suffix, label_nums):
  ...
  image_array_test, feat_array_test, label_array_test, test_data = get_test_Density(test_data_list, label_suffix)
  
  eva_path = "/data/lsl/SSpMV/models/prediction_result/Density_Reps/test_acc.txt"
  res_path = "/data/lsl/SSpMV/models/prediction_result/Density_Reps/predict_result.txt"
  
  # 保存模型的目录
  model_path = "/data/lsl/SSpMV/models/SpMV_DensityRep.keras"
  
  evaluate_MM_Adapter(model_path, image_array_test, feat_array_test, label_array_test, test_data, eva_path, res_path)
  
  metric_path = "/data/lsl/SSpMV/models/prediction_result/Density_Reps/metrics.res"
