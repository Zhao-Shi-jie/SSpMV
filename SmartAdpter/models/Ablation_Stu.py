...
class SpMV_Adapter(Model):
  ...
  def call(self, inputs):
    ...
    # x =  self.concatenate([x0, x1, x2])
    # x =  self.concatenate([x0, x1])
    # x =  self.concatenate([x0, x2, x3])
    x = self.dense1(x)
    x = self.dropout(x)
    x = self.dense2(x)
    x = self.dropout(x)
...
def train_MM_model(model_path, 
  ...
  logger_name = "Ablation_" + ttime +'.csv'
  ...
  print ("Finish Ablation Study Saving")
...
def train_and_get_res(training_data_list, val_data_list, label_suffix, label_nums):
  ...
  # 保存模型的目录
  model_path = "/data/lsl/SSpMV/models/Ablation.keras"
...
def test_model(test_data_list, label_suffix, label_nums):
  ...
  eva_path = "/data/lsl/SSpMV/models/prediction_result/MM_Adapter/Abl_test_acc.txt"
  res_path = "/data/lsl/SSpMV/models/prediction_result/MM_Adapter/Abl_predict_result.txt"
  ...
  metric_path = "/data/lsl/SSpMV/models/prediction_result/MM_Adapter/Abl_metrics.res"
