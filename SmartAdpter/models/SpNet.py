...
class DeepModel(Model):
    def __init__(self):
        ...
        self.conv1 = Conv2D(96, (11, 11), strides=(4, 4), padding='valid', activation='relu')
        self.pool1 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding='valid')  # 添加适当的池化
        self.conv2 = Conv2D(256, (5, 5), strides=(2, 2), padding='valid', activation='relu')  # 修改为5x5卷积核
        self.pool2 = MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding='valid')  # 添加适当的池化
        self.conv3 = Conv2D(256, (3, 3), strides=(1, 1), padding='same', activation='relu')  # 修改步长和填充
        self.conv4 = Conv2D(256, (3, 3), strides=(1, 1), padding='same', activation='relu')  # 保持 consistent
        self.pool4 = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid')  # 最终的池化层
        self.flatten = Flatten()
        self.dense1 = Dense(4096, activation='relu')  # 根据需要激活函数
        # self.dense2 = Dense(1000, activation='softmax')  # 假设有1000个类别
...
class SpNet(Model):
  def __init__(self, num_of_labels, *args, **kwargs):
    super(SpNet, self).__init__(*args, **kwargs)
  ...
  def call(self, inputs):
    ...
    x = self.final_dense(x1)
    return x
  
  def get_config(self):
    # 需要包括所有初始化参数
    config = super(SpNet, self).get_config()
...
def train_MM_model(model_path, 
  ...
  print ("Finish SpNet Model Saving")
...
#   print("CB_arr shape   : ", CB_array.shape)
  ...
  # 保存模型的目录
  model_path = "/data/lsl/SSpMV/models/SpNet.keras"
...
def evaluate_MM_Adapter(model_path, image_array_test, Row_Block_array_test, Col_Block_array_test, feat_array_test, label_array_test, test_data, eva_path, res_path):
  ...
  f_eva.write("SpNet Evaluating Acc:" + str(test_acc) + "\n")
  f_eva.close()
...
def test_model(test_data_list, label_suffix, label_nums):
  ...
  eva_path = "/data/lsl/SSpMV/models/prediction_result/SpNet/test_acc.txt"
  res_path = "/data/lsl/SSpMV/models/prediction_result/SpNet/predict_result.txt"
  ...
  metric_path = "/data/lsl/SSpMV/models/prediction_result/SpNet/metrics.res"
